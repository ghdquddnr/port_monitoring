...
- [x] 4.0 UI 컴포넌트 및 페이지 개발
  ...
  - [x] 4.7 토스트 알림 컴포넌트 (`app/components/Toast.tsx`: 성공/에러 메시지 표시)
  - [x] 4.8 확인 다이얼로그 컴포넌트 (`app/components/ConfirmDialog.tsx`: 제목, 메시지, 확인/취소)
  - [x] 4.9 헤더 컴포넌트 구현 (`app/components/Header.tsx`: 앱 제목, 테마 전환, 로그아웃)
  - [x] 4.10 검색 바 컴포넌트 구현 (`app/components/SearchBar.tsx`: 검색 입력, 프로토콜 필터)
  - [x] 4.11 새로고침 컨트롤 컴포넌트 (`app/components/RefreshControl.tsx`: 자동 ON/OFF, 간격 선택, 수동 버튼)
  - [x] 4.12 포트 데이터 fetching 커스텀 훅 (Dashboard에 통합 구현)
  - [x] 4.13 자동 새로고침 커스텀 훅 (Dashboard에 통합 구현)
  - [x] 4.14 포트 테이블 컴포넌트 구현 (`app/components/PortTable.tsx`: 테이블 렌더링, 액션 버튼)
